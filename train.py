...
if __name__ == '__main__':
    ...
    dataset, dataset_test = get_dataloaders(config, dir_path)
    ...
    filtered_dataset_test, stats = filter_dataset(
        dataset_test,
        min_num_nodes=config["filter"]["min_num_nodes"],
        max_num_nodes=config["filter"]["max_num_nodes"],
        min_label=config["filter"]["min_label"],
    ...
    print()
    print("====================")
    print(f"Number of graphs: {len(filtered_dataset_test)}")
    print(f"Statistics: {stats}")
    ...
    if config["features"]["numerical_features"] != []:
        ...
        filtered_dataset_test, scaler = normalize_numerical_features(filtered_dataset_test, scaler=scaler)
    else:
        scaler = None
        print("No numerical features to normalize in the test dataset.")

    train_size = int(config["training"]["split_ratio"] * len(filtered_dataset))
    ...
    test_loader = DataLoader(filtered_dataset_test, batch_size=config["training"]["batch_size"], shuffle=False)
    ...
    print(f"Number of test graphs: {len(filtered_dataset_test)}")
